...
<body>
	...
	<div class="content">
		...
		<div class="main">
			...
			<div class="m_llist" id="work-list">
				<c:forEach var="work" items="${list}">
					<div class="flex round page-item">
						<div>${work.work_id}</div>
						<div>
							<fmt:formatDate value="${work.work_write}" pattern="yyyy-MM-dd" />
						</div>
						<div>
							<fmt:formatDate value="${work.work_endate}" pattern="yyyy-MM-dd" />
						</div>
				...
				<div class="popup-content">
					...
					<form id="registration-form" method="post" action="insertWork">
						<input type="hidden" name="action" value="add"> <input
							type="hidden" name="user_id" value="박경민"> <label
							for="work_id">작업지시 NO:</label><br> <input type="text"
							id="work_id" name="work_id" placeholder="작업지시 NO" required><br>

						<label for="work_write">작성일자:</label><br> <input type="date"
							id="work_write" name="work_write" required><br> <label
							for="work_endate">납기일자:</label><br> <input type="date"
							id="work_endate" name="work_endate" required><br> <label
							for="work_name">품목명:</label><br> <input type="text"
							id="work_name" name="work_name" placeholder="품목명" required><br>

						<label for="item_code">품목코드:</label><br> <select
							id="item_code" name="item_code" required>
							<option value="">품목코드를 선택하세요</option>
							<!-- 기본 옵션 -->
						...
						</select><br> <label for="production_qty">수량:</label><br> <input
							type="number" id="production_qty" name="production_qty"
							placeholder="수량" required><br> <input type="submit"
							id="add-button1" value="저장">
					</form>
				</div>
			</div>
		</div>
